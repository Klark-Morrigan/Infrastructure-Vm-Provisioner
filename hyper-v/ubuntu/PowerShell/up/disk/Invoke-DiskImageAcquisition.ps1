...
function Invoke-DiskImageAcquisition {
    ...
    # after cloud-config.service). See Invoke-BaseImagePatch.ps1 for the
    # full implementation. The sentinel name encodes the patch revision:
    #   .nocloud-patched     - original (only NoCloud datasource)
    #   .image-patched       - v1 of Patch 2, used cloud-init.target -
    #                          BROKEN: sshd never started
    #   .image-patched-v2    - drop-in on ssh.service AND ssh.socket -
    #                          BROKEN: ordering cycle, cloud-config
    #                          dropped half the time
    #   .image-patched-v3    - drop-in on ssh.service, ssh.socket
    ...
    #                          left untouched
    #   .image-patched-v5    - + Patch 3, empty 90-hotplug-azure.yaml so
    #                          the router's static netplan wins at init-local
    #                          and ext0 holds its static IP before the direct
    #                          SSH probe runs
    #   .image-patched-v6    - current: + Patch 4, `acl` baked into the base
    #                          image so the runner register play's ~61s apt
    #                          install detects it present and no-ops
    ...
    # ------------------------------------------------------------------
    $patchedSentinel = $baseImagePath -replace '\.vhdx$', '.image-patched-v6'
    Invoke-WithSubStepTimer `
        -Parent 'Disk image acquisition' `
