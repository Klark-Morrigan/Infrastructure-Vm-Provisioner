...
.DESCRIPTION
    ...
    vault initialisation) so it cannot be dot-sourced safely from a test.
    The harness follows the same pattern as start-vms.Tests.ps1: copy the
    script into a temp dir alongside empty stub files for every
    dot-source it performs, then invoke the shim through `& $shimPath`.

    The mocked seam is Initialize-MicrosoftPowerShellSecretStoreVault -
    the SecretManagement bootstrap is not on the box, and the assertions
    care only about what *this* script forwards to it:

      - SecretName is built as `VmProvisionerConfig-<SecretSuffix>` so
...
BeforeAll {
    $script:realPath = Join-Path $PSScriptRoot '..\..\hyper-v\ubuntu\shared\setup-secrets.ps1'

    # Sibling shim directory; empty stub files satisfy each dot-source
    # in the script. The function-shaped seams are stubbed in this
    # BeforeAll so Pester's Mock can attach to them. The shim mirrors the
    # repo's shared/ + PowerShell/ slice layout so the script's
    # `$PSScriptRoot\..\PowerShell\...` reach into the reconciler resolves
    # exactly as it does in production.
    $script:shimDir = Join-Path ([IO.Path]::GetTempPath()) `
        ("setup-secrets-test-" + [Guid]::NewGuid().ToString('N'))
    New-Item -ItemType Directory `
        -Path (Join-Path $script:shimDir 'PowerShell\common\config') `
        -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $script:shimDir 'shared') `
        -Force | Out-Null

    foreach ($rel in @(
        'PowerShell\common\config\ConvertFrom-VmConfigJson.ps1',
        'PowerShell\Install-ModuleDependencies.ps1'
    )) {
    ...
    Copy-Item -LiteralPath $script:realPath `
        -Destination (Join-Path $script:shimDir 'shared\setup-secrets.ps1')
    $script:shimPath = Join-Path $script:shimDir 'shared\setup-secrets.ps1'
    ...
    # ConvertTo-Array is called inside the -Validate block. The block is
    # never executed by the stubbed cmdlet, so the stub is only here so
    # the script's dot-source path resolves without errors at the host
    # session level. (No-op return matches the real signature.)
...
Describe 'setup-secrets.ps1 - vault initialisation wiring' {
    ...
    # ------------------------------------------------------------------
        ...
        It 'forwards SecretName as VmProvisionerConfig dash suffix' {
            ...
            Should -Invoke Initialize-MicrosoftPowerShellSecretStoreVault `
                -Times 1 -Exactly -ParameterFilter {
                    ...
                    $SecretName -eq 'VmProvisionerConfig-Production'
                }
        }
        ...
        It 'uses the literal suffix without normalising case or punctuation' {
            ...
            Should -Invoke Initialize-MicrosoftPowerShellSecretStoreVault `
                -Times 1 -Exactly -ParameterFilter {
                    $SecretName -eq 'VmProvisionerConfig-CI-pr-42'
                }
        }
    }
...
Describe 'setup-secrets.ps1 - SecretSuffix parameter contract' {
    ...
    It 'marks -SecretSuffix ValidateNotNullOrEmpty' {
        ...
        $hasValidator | Should -Not -BeNullOrEmpty `
            -Because 'an empty suffix would collide every lifecycle on "VmProvisionerConfig-"'
    }
}
