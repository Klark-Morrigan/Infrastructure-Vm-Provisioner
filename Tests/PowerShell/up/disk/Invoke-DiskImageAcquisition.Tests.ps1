...
BeforeAll {
    ...
    . "$PSScriptRoot\..\..\..\..\hyper-v\ubuntu\PowerShell\up\disk\Invoke-DiskImageAcquisition.ps1"
    ...
    function Set-DefaultTestPath {
        Mock Test-Path {
            ...
            if ($Path -match '\.image-patched-v6$')                { return $true  }
    ...
    # ------------------------------------------------------------------
        ...
        It 'calls Invoke-BaseImagePatch with the base image and sentinel paths' {
            ...
            Should -Invoke Invoke-BaseImagePatch -Times 1 -Exactly -ParameterFilter {
                ...
                $SentinelPath  -eq 'C:\VHDs\ubuntu-24.04-server-cloudimg-amd64.image-patched-v6'
            }
        }
