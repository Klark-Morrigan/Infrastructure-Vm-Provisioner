...
#   2. This repo's composer ansible-lint (run-lint-ansible.sh), run through
#      the shared Common-Ansible controller venv.
...
# the composer playbook linted on a local pre-push run. The bats test half is
# run-tests-bash.sh; run-ci-yaml-and-bash.sh runs both. Common-Automation is
